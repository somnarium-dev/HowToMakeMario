...
createFireball = function()
{
	...
	// Create the fireball, pointing it so that it travels along the shooting angle.
	var new_fireball = instance_create_layer(x + x_offset, y + y_offset, "Projectiles", obj_piranha_spitter_fireball, {direction: shooting_angle});
	
	// Then snap its starting position to an integer.
	new_fireball.x = round(new_fireball.x);
	new_fireball.y = round(new_fireball.y);
}